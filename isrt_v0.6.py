...
#v0.6 - Integration further functionalities and Layout Cleanup - added direct rcon command buttons
...
from gui.histmgr_gui import Ui_histmgr_window
...
#PyQt5 HistMgr UI
class histmgr_gui(QtWidgets.QWidget):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.histgui = Ui_histmgr_window()
        self.histgui.setupUi(self)

        self.histgui.btn_histmgr_close.clicked.connect(self.closeapp)

    def closeapp(self):
        refresh_command_history_dropdown_box()
        self.close()

    #Action on CloseEvent - Update Dropdown Command History
...
class maingui(QtWidgets.QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        #Define menu action items including their functionalities
        ...
        self.gui.menu_action_histmgr.triggered.connect(self.show_histmgr_app)
    ...
    #Show the History Manager Window
    def show_histmgr_app(self):
        self.histmgr_app = None
        if self.histmgr_app is None:
            self.histmgr_app = histmgr_gui()
        self.histmgr_app.show()
...
if __name__ == '__main__':
    ...
    mgui = maingui()
    mgui.show()
    #Ensure Update of Server Alias DropDown Menu in maingui while switching between guis
    def refresh_dropdown_box():
        mgui.fill_dropdown_box()
    #Ensure Update of Command History DropDown Menu in maingui while switching between guis
    def refresh_command_history_dropdown_box():
        mgui.fill_dropdown_history_box()
    sys.exit(app.exec_())
