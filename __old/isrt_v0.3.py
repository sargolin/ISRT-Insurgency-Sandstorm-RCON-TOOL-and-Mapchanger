...
#v0.3 - Integration further functionalities and database integration
...
from gui.isrt_main_gui import Ui_MainWindow
from gui.about_gui import Ui_aboutwindow
from gui.help_gui import Ui_helpwindow
import socket
...
#Check for the format string and go for the rcon command, but only if rcon port and rcon password are given
    def checkandgorcon(self):
        ...
        if self.gui.entryrconpw.text():
            ...
            self.gui.terminalwindows.setText("No RCON Password given - please retry!")
    ...
    def checkandgoquery(self):
        ...
        if (re.search(self.regexip, self.gui.entryip.text())):  
            ...
            try:
                ...
                self.gui.terminalwindows.setText(self.gui.entryqueryport.text() + " is no valid Port number - please retry!")
        else:  
            self.gui.terminalwindows.setText(self.gui.entryip.text() + " is no valid IP address - please retry!")  
...
#Execute RCON Command, when called by checkandgorcon()!
    def rconserver(self, serverhost, rconpassword, rconport, rconcommand):
        if rconcommand.startswith("say") or rconcommand.startswith("Say") or rconcommand.startswith("SAY"):
            ...
            self.gui.terminalwindows.setText("Say Commands do not provide a feedback, but are sent nevertheless correctly if no error is thrown!")
        else:
            console = Console(host=serverhost, password=rconpassword, port=rconport)
